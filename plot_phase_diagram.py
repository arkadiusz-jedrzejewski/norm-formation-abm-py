...
is_symmetric = int(params[12])
num = 100
if mode == 0:
    p_start, p_end, _ = params[1], params[2], int(params[3])
    q, k, m, x0 = params[5], params[6], params[7], params[8]

    plot_fixed_points_p(p_tab=np.linspace(0.0001, 1, num), q=q, k=k, m=m, x0=x0, is_quenched=not is_annealed,
    ...
    plot_fixed_points_k(k_tab=np.linspace(k_start, k_end, num), q=q, p=p, m=m, x0=x0, is_quenched=not is_annealed,
                        is_symmetric=is_symmetric)
    ...
    plot_fixed_points_m(m_tab=np.linspace(m_start, m_end, num), q=q, p=p, k=k, x0=x0, is_quenched=not is_annealed,
                        is_symmetric=is_symmetric)
    ...
    plot_fixed_points_x0(x0_tab=np.linspace(x0_start, x0_end, num), q=q, p=p, k=k, m=m, is_quenched=not is_annealed,
                         is_symmetric=is_symmetric)
    plt.xlim([x0_start, x0_end])
    plt.xlabel("x0")
    plt.ylabel("a")

time_start = int(time_horizon*0.9)
...
np.savetxt(dir_name + f"/a_abs.csv", np.mean(cs, axis=1))
